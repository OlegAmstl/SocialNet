...
def user_login(request):
    '''
    Представление входа пользователя в поиложение.
    '''
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    login(request, user)
                    return HttpResponse('Вход в аккаунт выполнен успешно.')
                else:
...
def edit(request):
    '''
    Представление редактирования информации о пользователе.
    '''
    if request.method == 'POST':
...
def user_detail(request, username):
    '''
    Представление порофайла выбранного пользователя.
    '''
